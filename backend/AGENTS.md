...
Sections contributed by this deployment fork that are not part of the upstream
split files. Router-level detail lives in the nearest subdirectory guide.

- **YiXin transit / CAS SSO** (`app/gateway/transit/`, `app/gateway/auth/cas_auth.py`): CAS 2.0/3.0 single sign-on (config `auth.local.cas` in `config.yaml`) resolves a per-user oneai apiKey and default model live from YiXin; values are never stored, and `GET /api/v1/auth/me` surfaces `is_yixin_user` / `has_api_key` / `default_model`.
- **Operations dashboard** (`/api/operations`): admin-only platform KPI adapter — see the Routers table in [app/gateway/AGENTS.md](app/gateway/AGENTS.md).
- **Agent Management** (`/api/agents`, `/api/agent-management`): user-owned and public Agent catalog/settings surface — see the same Routers table.
- **Materials preview** (`/api/materials`): materials preview/search router mounted from `app/gateway/routers/materials.py`.
