...
| **Agent Management** (`/api/agents`, `/api/agent-management`) | Catalog, files, versions, validation, testing, activity, import/export, schedules, sharing, and structured settings for user-owned and public Agents. Management routes take `scope=user|platform`; public writes require an administrator. `PUT /api/agents/{name}/settings` updates the structured settings surface while preserving SOUL, guide questions, and unmanaged config fields. `skills` and `mcp_servers` use explicit allowlists, where `[]` means none and `null` means inherit. |
...
`get_available_tools(groups, include_mcp, model_name, subagent_enabled, *, mcp_servers=None)` assembles:
...
- Per-agent `mcp_servers` (in a custom agent's `config.yaml`) uses three-state semantics: omitted/`null` exposes all enabled MCP servers, `[]` exposes none, and a list exposes only the named servers. The allowlist is inherited by task subagents and forwarded to ACP sessions.
