...
- **`app/`** — Next.js App Router. Routes include `/` (hidden landing route — `src/app/page.tsx` server-side redirects it to `/workspace/chats/new` when authenticated, `/login` otherwise; the landing components stay for blog/docs reuse), `/showcase/[thread_id]` (allowlisted public read-only demos), `/workspace/chats/[thread_id]` (authenticated chat), `/workspace/agents/[agent_name]` and `/workspace/agents/new` (custom agents), `/workspace/skills` (skills gallery ported from the ai-agent-harness fork — components live under `components/workspace/skills/`, backed by `core/skills/extended.ts` (new-file API + hooks + Skill type augmentation) and `core/skills/categories.ts`, leaving the original `core/skills/{api,hooks,type}.ts` untouched), `/workspace/operations` (admin-only operations dashboard backed by `/api/operations/dashboard` plus deferred `/api/operations/dashboard/details`), `/artifacts/view` (chrome-free window that renders Markdown or CSV/TSV artifacts with the panel's own renderer), `/blog/…`, the `(auth)/{login,setup,auth/callback}` flow, `/[lang]/docs/…`, and `/api/…` route handlers (e.g. `/api/memory`).
...
Covered in [src/AGENTS.md](src/AGENTS.md#data-flow); the stream-reconnect / history-page / composer `/goal` semantics live there. Digital-human and voice-input composer helpers are documented below.

### Key Patterns

Covered in [src/AGENTS.md](src/AGENTS.md#key-patterns) (SSE replay gaps, stream-mode sanitizing, subtask step history).

### Interaction Ownership

Covered in [src/AGENTS.md](src/AGENTS.md#interaction-ownership); more specific `AGENTS.md` files under `src/` contain the split sections.
